...
- [high] Fixed OAuth redirect bug by changing token expiry validation from 30 minutes to 10 minutes in src/auth/callback.ts to match src/auth/session.ts implementation. Test suite initially failed with exit code 1, then passed after correction.
